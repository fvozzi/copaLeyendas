...
export function ProgramScenarioPreviewTable({ preview, config, courts, venues, busy, onOverride, onEditing }: {
  ...
  const shown = slots.filter((slot) => (!onlyPending || preview.warnings.some((w) => w.sequence === slot.sequence)) && (!search.trim() || `P${slot.sequence} ${slot.tournamentCategory.category.name} ${slot.zoneName}`.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase())));
  ...
  return <div className="scenario-preview">
    ...
    {shown.map((slot) => {
      ...
      return <ScenarioMatchRow key={slot.sequence} slot={slot} override={override} venue={venue} day={day} plannedCourtId={override?.courtId ?? rule?.courtId ?? null} courts={courts} busy={busy} warning={preview.warnings.find((w) => w.sequence === slot.sequence)?.message} onOverride={(sequence, value) => { closeEditor(); onOverride(sequence, value); }} editing={editingSequence === slot.sequence} onToggle={() => { const next = editingSequence === slot.sequence ? null : slot.sequence; setEditingSequence(next); onEditing(next !== null); }} />;
    })}
  </div>;
}

function ScenarioMatchRow({ slot, override, venue, day, plannedCourtId, courts, busy, warning, onOverride, editing, onToggle }: {
  ...
  return <section className={`scenario-match-row${warning ? ' has-warning' : ''}`} id={`scenario-match-${slot.sequence}`}>
