...
export function AdminLocalitiesPage() {
  ...
  return <div className="admin-panel">
    ...
    <section className="data-card localities-data-card"><AdminDataGrid columns={[
    ...
    ]} rows={localities} renderActions={(item) => <div className="locality-row-actions"><div><button type="button" className="inline-link" onClick={() => openEdit(item)}>Editar</button><button type="button" className="danger-link" disabled={deletingId !== null} onClick={() => { setDeleteError(null); setPendingDelete(item); }}>{deletingId === item.id ? 'Eliminando...' : 'Eliminar'}</button></div>{deleteError?.id === item.id ? <small className="admin-grid-detail" role="alert">{deleteError.message}</small> : null}</div>} emptyMessage="No hay localidades cargadas." preserveTableOnMobile /></section>
