...
import { createCashExpense, deleteCashExpense, getCash, updateCashFee } from '../lib/api';
...
export function AdminCashPage() {
  ...
  const load = () => getCash().then((data) => { setSummary(data); setFee(String(data.feePerPlayer)); }).catch((reason: Error) => setError(reason.message));
  useEffect(() => { load(); }, []);

  const saveFee = async (event: React.FormEvent) => {
    event.preventDefault();
    const value = Number(fee);
    if (!Number.isInteger(value) || value < 0) { setError('Ingresa una tarifa valida.'); return; }
    setSaving(true);
    try { await updateCashFee(value); load(); } catch (reason) { setError(reason instanceof Error ? reason.message : 'No se pudo actualizar la tarifa.'); } finally { setSaving(false); }
  };
  ...
  return <div className="admin-panel">
    ...
    <section className="data-card"><div className="panel-header"><div><h2>Tarifa de inscripcion</h2><p className="field-hint">Se aplica por jugadora a las parejas pagas.</p></div></div><form className="inline-form" onSubmit={saveFee}><label>Valor por jugadora<input type="number" min="0" step="1" value={fee} onChange={(event) => setFee(event.target.value)} required /></label><button className="secondary-button" disabled={saving}>Guardar tarifa</button></form></section>
