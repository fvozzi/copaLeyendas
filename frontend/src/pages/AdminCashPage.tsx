...
import { createCashExpense, deleteCashExpense, getCash, openRegistrationPaymentProof } from '../lib/api';
...
export function AdminCashPage() {
  ...
  return <div className="admin-panel">
    ...
    <section className="data-card"><div className="panel-header"><div><h2>Ingresos</h2><p className="field-hint">Inscripciones no bonificadas con comprobante de pago.</p></div></div><AdminDataGrid rows={summary?.incomes ?? []} emptyMessage="Todavia no hay ingresos registrados." columns={[{ label: 'Equipo / localidad', render: (item) => <strong>{item.team}</strong> }, { label: 'Concepto', render: (item) => item.concept }, { label: 'Jugadoras', render: (item) => item.players }, { label: 'Monto', render: (item) => money.format(item.amount) }, { label: 'Fecha', render: (item) => new Date(item.paidAt).toLocaleDateString('es-AR') }, { label: 'Comprobante', render: (item) => <button className="inline-link" onClick={() => openRegistrationPaymentProof(item.registrationId, item.id).catch((reason: Error) => setError(reason.message))}>Ver</button> }]} /></section>
