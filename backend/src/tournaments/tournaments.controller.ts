...
export class TournamentsController {
  ...
  @Patch('categories/:id') updateCategory(@CurrentUser() user: AuthenticatedUser, @Param('id', ParseIntPipe) id: number, @Body() dto: { pointsPerSet?: number; setsToWin?: number; zoneSize?: number; registrationsOpen?: boolean }) { this.assertDirector(user); return this.s.updateCategory(id, dto); }
  ...
  @Patch('zones/:id') updateZone(@CurrentUser() user: AuthenticatedUser, @Param('id', ParseIntPipe) id: number, @Body() dto: { name?: string; courtId?: number; capacity?: number }) { this.assertDirector(user); return this.s.updateZone(id, dto); }
  ...
  @Patch('schedule-slots/:id') scheduleSlot(@CurrentUser() user: AuthenticatedUser, @Param('id', ParseIntPipe) id: number, @Body() dto: { scheduledAt?: string | null }) { this.assertDirector(user); return this.s.updateScheduleSlot(id, dto); }
